...
python3 ./process_data_inference.py --input_img "$disaster_post_file" --label_path "$label_temp"/"${input_image%.*}".json --output_dir "$inference_base"/output_polygons --output_csv "$inference_base"/output.csv 
...
python3 "$XBDIR"/utils/combine_jsons.py --polys "$label_temp"/"${input_image%.*}".json --classes /tmp/inference/classification_inference.json --output "$inference_base/inference.json" 
printf "\n"

# Transforming the inference json file to the image required for scoring
printf "Finalizing output file" 
python3 "$XBDIR"/utils/inference_image_output.py --input "$inference_base"/inference.json --output "$output_file" 
